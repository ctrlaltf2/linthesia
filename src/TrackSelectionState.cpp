...
void TrackSelectionState::Init() {
    ...
    for (size_t i = 0; i < m.Tracks().size(); ++i) {
        ...
        Track::TrackColor
            color = static_cast<Track::TrackColor>((m_track_tiles.size()) % Track::UserSelectableColorCount);
...
void TrackSelectionState::Update() {
    ...
    for (size_t i = start; i < end; ++i) {
        ...
        if (t.ButtonLeft().hovering || t.ButtonRight().hovering) {
            ...
            switch (t.GetMode()) {
                case Track::ModeNotPlayed:m_tooltip = "Track won't be played or shown during the game.";
                    break;

                case Track::ModePlayedAutomatically:m_tooltip = "Track will be played automatically by the game.";
                    break;
                ...
                case Track::ModeYouPlay:m_tooltip = "'You Play' means you want to play this track yourself.";
                    break;

                case Track::ModeYouPlaySilently:m_tooltip = "Same as 'You Play', ignore velocity from MIDI.";
                    break;

                case Track::ModeLearning:m_tooltip = "Wait for you to play.";
                    break;

                case Track::ModeLearningSilently:m_tooltip = "Wait for you to play, do not produce sounds from MIDI.";
                    break;
            ...
            if (t.IsPreviewOn()) {
                ...
                for (size_t i = 0; i < track.Events().size(); ++i) {
                    ...
                    if (ev.Type() == MidiEventType_NoteOn && ev.NoteVelocity() > 0) {
                        additional_time +=
                            track.EventUsecs()[i] - m_state.midi->GetDeadAirStartOffsetMicroseconds() - 1;
                        break;
                    }
                }
